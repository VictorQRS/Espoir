...
from entities.battle_denial_reason import BattleDenialReason
from entities.battle_result import BattleResult
from entities.card_type import CardType
from entities.leave_denial_reason import LeaveDenialReason
...
def simulateBattles(context):
    ...
    for i in range(0, numSimultaneousBattles):
        player1 = NonPlayablePlayer()
        player2 = NonPlayablePlayer()
        ...
        context.Players = context.Players.filter(lambda p: p.Stars > 0 and len(p.Cards) > 0)
